        ...
        <div class="element-wrapper">
            <div class="element-box">
                ...
                <div class="element-actions" style="margin-top: -2.2rem;">
                    <button class="btn btn-success" data-target="#addResellerModal" data-toggle="modal">Add Reseller</button>
                </div>
                <div class="form-desc">
                    &nbsp;
                </div>
...
<div aria-hidden="true" aria-labelledby="exampleModalLabel" class="modal fade" id="addResellerModal" role="dialog" tabindex="-1">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="orderForm" role="add-reseller-form" action="<?php echo e(url('admin/reseller/add-reseller')); ?>" data-DataTables="datatable" method="POST">
                    <?php echo csrf_field(); ?>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-6 col-sm-12 mb-1">
                                <label class="col-form-label" for="account-old-password">Email</label>
                                   <input class="form-control" type="text" name="email">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6 col-sm-12 mb-1">
                                <label class="col-form-label" for="account-old-password">Password</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <input class="btn btn-primary" type="submit" data-request="ajax-submit" data-target='[role="add-reseller-form"]' value="Submit" />
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        var url = "<?php echo e(custom_secure_url('admin/fetch')); ?>/Resellers/0";
        var onDraw = function() {};
        var options = [ {
            "orderable": false,
            "searchable": false,
            ...
            }, 
            {
                ...
                "orderable": false,
                render: function(data, type, full, meta) {
                    return '<a href="reseller/userdetails/' + full.id + '" class="edit btn btn-primary btn-sm"> <i class="os-icon os-icon-eye"></i></a>';
                }
            }
        ];
...
$.ajax({
    url:"<?php echo e(url('admin/reseller/addReseller/')); ?>/"+id,
    type:"get",
    ...
    success:function(resp){
        ...
        {
            $('#addResellerModal input[name=name]').val(resp.data.name);
            $('#addResellerModal input[name=email]').val(resp.data.email);
            $('#addResellerModel input[name=password').val(resp.data.password);
            $('#addResellerModal select[name=status]').val(resp.data.status);
            $('#addResellerModal input[name=id]').val(resp.data.id);
            $('#addResellerModal').modal('show');
        }
    }
});
});
...
<?php echo $__env->make('layouts.admin.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\xettle-main\resources\views/admin//reseller.blade.php ENDPATH**/ ?>