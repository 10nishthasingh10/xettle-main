    ...
    <div class="content-box">
        <div class="element-wrapper">
            <div class="element-box">
                ...
                <form id="rv-txn-form" role="rv-txn-us-form" action="<?php echo e(url('admin/dispute-transactions/smart-collect/submit')); ?>" data-DataTables="datatable" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="smcl_utr" class="form-control" required />
                                </div>
                            </div>
                ...
                <div class="row">
                    <div class="col-md-12 d-none" id="utr-response-data">
                        ...
                        <form id="rv-txn-form-final" role="rv-txn-us-form-final" action="<?php echo e(url('admin/dispute-transactions/smart-collect/final-submit')); ?>" data-DataTables="datatable" method="POST">
                            <fieldset class="form-group">
                                <?php echo csrf_field(); ?>
                                <div class="col-md-12">
                                    <div class="col-md-12 text-right">
                                        <input type="hidden" name="utr_final" value="" id="utr_final">
...
<script id="details-template" type="text/x-handlebars-template">
    <table class="expandtable">
        <tr>
            <td><b>Payer VPA :</b> {{remitter_vpa}}</td>
            <td><b>Payer Name :</b> {{remitter_name}}</td>
            <td><b>Payer A/c No :</b> {{remitter_account}}</td>
            <td><b>Payer IFSC :</b> {{remitter_ifsc}}</td>
        </tr>
        <tr>
            <td><b>Account ID :</b> {{v_account_id}}</td>
            <td><b>Reference ID :</b> {{reference_id}}</td>
            <td><b>Credit Ref No :</b> {{credit_ref_no}}</td>
            <td><b>Txn Date :</b> {{payment_time}}</td>
        </tr>
        <tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var url = "<?php echo e(custom_secure_url('admin/dispute-transactions/report/smart-collect')); ?>";
        var onDraw = function() {};
        var options = [{
    ...
    function callbackDispute(response) {
        if (response.code === "0x0200") {
            ...
            let res = `
                    <tr>
                        <td><b>UTR :</b> ${response.data.utr}</td>
            ...
            if (response.data.is_vpa === '1') {
                res += `<td><b>Payee VPA :</b> ${response.data.virtual_vpa_id}</td>`;
            } else {
                res += `<td><b>Payee VAN :</b> ${response.data.v_account_number}</td>`;
            }

            res += `
                        <td><b>Account ID :</b> ${response.data.v_account_id}</td>
                        <td><b>Transaction Date :</b> ${response.data.payment_time}</td>
                        <td><b>Credited At :</b> ${response.data.trn_credited_at}</td>
                    </tr>
                    <tr>
                        <td><b>Payer VPA :</b> ${response.data.remitter_vpa}</td>
                        <td><b>Payer Name :</b> ${response.data.remitter_name}</td>
                        <td><b>Payer A/c No :</b> ${response.data.remitter_account}</td>
                        <td><b>Payer IFSC :</b> ${response.data.remitter_ifsc}</td>
                    </tr>
                    `;

            $('#utr-response-data').removeClass('d-none');
            $('#utr-response-data table tbody').html(res);
            $('#utr_final').val(response.data.utr);
        }
    }
</script>

<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\xettle-main\resources\views/admin/disputed_smart_collect_txns.blade.php ENDPATH**/ ?>