...
function getValidationGraph(start, end, type) {
    showChartOverlay('#virtualAccountChartOverlay');
    ...
    $.post($('meta[name="base-url"]').attr('content') + '/graphs/virtual-account', jsonData, function (response) {
        if (response.code === '0x0200') {

            drawValidationGraph(response);
            hideChartOverlay('#virtualAccountChartOverlay');
...
function drawValidationGraph(response) {
    ...
    if (showGraph) {
        ...
        $('#virtualAccountChart').html(`<div class="position-relative mb-4">
            <canvas id="virtualAccountCanvas" height="220"></canvas>
        </div>`);

        var virtualAccountCanvas = $('#virtualAccountCanvas');
        new Chart(virtualAccountCanvas, {
            data: {
                labels: labels,
                datasets: [
                    {
