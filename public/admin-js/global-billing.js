...
$(function () {
  ...
  $("#add_new_product_row").on("click", function () {
    $("#modal-product-table tbody").append(
      addNewProductRow(productCountFromDb++)
    );
  ...
  $("#add_new_product_fee_row").on("click", function () {
    $("#modal-product-fee-table tbody").append(
      addNewProductFeeRow(productFeeCountFromDb++)
    );
  ...
  let serviceOptionsDt = [
    ...
    },
    {
      ...
      render: function (data, type, full) {
        if (data === "1") {
          return `<div class="icon-state"><label class="switch serviceStatus" data-id="${full.id}"><input type="checkbox" checked><span class="switch-state"></span></label></div>`;
        } else {
          return `<div class="icon-state"><label class="switch serviceStatus" data-id="${full.id}"><input type="checkbox"><span class="switch-state"></span></label></div>`;
        }
      },
    },
    {
      ...
      render: function (data, type, full) {
        if (data === "1") {
          return `<div class="icon-state"><label class="switch serviceActivation" data-id="${full.id}"><input type="checkbox" checked><span class="switch-state"></span></label></div>`;
        } else {
          return `<div class="icon-state"><label class="switch serviceActivation" data-id="${full.id}"><input type="checkbox"><span class="switch-state"></span></label></div>`;
        }
      },
    },
    {
    ...
    },
    {
      ...
      render: function (data, type, full) {
        ...
        actionBtn += `<button title="Product List" data-id="${full.service_id}" data-servicename="${full.service_name}" class="btn btn-link border btn-sm addUpdateProducts"><i class="os-icon os-icon-list"></i></button>`;
  ...
  datatableSetup(
    $('meta[name="base-url"]').attr("content") +
      "/admin/global-billing/data-table/service-list",
    serviceOptionsDt,
    () => {},
    "#dt-service-list"
  );
  ...
  let schemeInfoOptionsDt = [
    ...
    },
    {
      data: "start_value",
      render: function (data, type, full, meta) {
        if (full.start_value === null || full.start_value === "") return "";
    ...
    },
    {
      data: "end_value",
      render: function (data, type, full, meta) {
        if (full.end_value === null || full.end_value === "") return "";
    ...
    },
    {
      data: "fee",
      render: function (data, type, full, meta) {
        if (full.fee === null || full.fee === "") return "";
    ...
    },
    {
      data: "min_fee",
      render: function (data, type, full, meta) {
        if (full.min_fee == null || full.min_fee == "") return "";
    ...
    },
    {
      data: "max_fee",
      render: function (data, type, full, meta) {
        if (full.max_fee === null || full.max_fee === "") return "";
  ...
  $("form#searchForm_2").on("submit", function (e) {
    ...
    if (isFirstReqSchemeInfo) {
      ...
      datatableSetup(
        $('meta[name="base-url"]').attr("content") +
          "/admin/global-billing/data-table/product-and-fee",
        schemeInfoOptionsDt,
        () => {},
  ...
  $("#dt-service-list").on("click", ".viewServiceProducts", function () {
    ...
    if (!viewServiceProductsAjax) {
      ...
      $.ajax({
        url:
          $('meta[name="base-url"]').attr("content") +
          `/admin/global-billing/products/fetch-list`,
        type: "POST",
        data: {
          serviceId: serviceId,
  ...
  $("#dt-service-list").on("click", "label.serviceActivation", function () {
    if (!serviceActivationAjax) {
      ...
      $.ajax({
        url:
          $('meta[name="base-url"]').attr("content") +
          `/admin/global-billing/status/service-activation/${rowId}`,
        type: "GET",
  ...
  $("#dt-service-list").on("click", "label.serviceStatus", function () {
    if (!serviceStatusAjax) {
      ...
      $.ajax({
        url:
          $('meta[name="base-url"]').attr("content") +
          `/admin/global-billing/status/service-status/${rowId}`,
        type: "GET",
        success: function (response) {
          Swal.fire({
            title: response.title,
  ...
  $("#modal-product-table").on("click", "label.productStatus", function () {
    if (!productStatusAjax) {
      ...
      $.ajax({
        url:
          $('meta[name="base-url"]').attr("content") +
          `/admin/global-billing/status/product-status/${rowId}`,
        type: "GET",
        success: function (response) {
          Swal.fire({
            title: response.title,
              ...
              if (
                ...
                response.data.forEach((row, index) => {
                  formInputRows += `
                                        <tr class="fromdb">
                                            <td>
                                                <input type="hidden" name="product_id[${productCountFromDb}]" value="${
                    row.id
                  }" />
                                                <input value="${
                                                  row.name
                                                }" type="text" name="product_name[${productCountFromDb}]" class="form-control productUpdateForm" />
                                            </td>
                                            <td><div class="form-control bg-light">${
                                              row.slug
                                            }</div></td>
                                            <td><input value="${
                                              row.min_order_value
                                            }" type="number" name="min_ord_value[${productCountFromDb}]" class="form-control productUpdateForm onlyNumberInput" /></td>
                                            <td><input value="${
                                              row.max_order_value
                                            }" type="number" name="max_ord_value[${productCountFromDb}]" class="form-control productUpdateForm onlyNumberInput" /></td>
                                            <td><input value="${
                                              row.tax_value
                                            }" type="number" name="tax_value[${productCountFromDb}]" class="form-control productUpdateForm onlyNumberInput" /></td>
                                            <td class="text-center">
                                                ${
                                                  row.is_active === "1"
                                                    ? `<div class="icon-state"><label class="switch productStatus" data-id="${row.id}"><input type="checkbox" checked><span class="switch-state"></span></label></div>`
                                                    : `<div class="icon-state"><label class="switch productStatus" data-id="${row.id}"><input type="checkbox"><span class="switch-state"></span></label></div>`
                                                }
                                            </td>
                                            <td class="text-center">
                                                <span class="inline-flex"><span title="Product Fee List" data-id="${
                                                  row.product_id
                                                }" data-servicename="${serviceName}" data-productname="${
                    row.name
                  }" class="btn btn-link border btn-sm addUpdateProductFee"><i class="os-icon os-icon-list"></i></span></span>
                                            </td>
                                        </tr>`;
                  productCountFromDb++;
                });
  ...
  $("#modal-product-table").on("click", ".addUpdateProductFee", function () {
    ...
    if (!addUpdateProductFeeAjax) {
      ...
      $.ajax({
        url:
          $('meta[name="base-url"]').attr("content") +
          `/admin/global-billing/products/fetch-fee-list`,
        type: "POST",
        data: {
          productId: productId,
          _token: $('meta[name="csrf-token"]').attr("content"),
              ...
              if (
                ...
                response.data.forEach((row, index) => {
                  formInputRows += `
                                    <tr class= "feeFromDb">
                                             <td>
                                                 <input type="hidden" name="fee_id[${productFeeCountFromDb}]" value="${
                    row.id
                  }" />
                                                 <input value="fixed" type="radio" name="fee_type[${productFeeCountFromDb}]" id="feeTypeF${productFeeCountFromDb}" class="form-control-radio productFeeUpdateForm" ${
                    row.type === "fixed" ? "checked" : ""
                  } data-value="${row.type}" />
                                                 <label for="feeTypeF${productFeeCountFromDb}">Fixed</label>
                                                 <input value="percent" type="radio" name="fee_type[${productFeeCountFromDb}]" id="feeTypeP${productFeeCountFromDb}" class="form-control-radio productFeeUpdateForm" ${
                    row.type === "percent" ? "checked" : ""
                  } data-value="${row.type}" />
                                             ...
                                             <td><input value="${
                                               row.start_value
                                                 ? row.start_value
                                                 : ""
                                             }" type="number" name="fee_start_value[${productFeeCountFromDb}]" class="form-control productFeeUpdateForm onlyNumberInput" /></td>
                                             <td><input value="${
                                               row.end_value
                                                 ? row.end_value
                                                 : ""
                                             }" type="number" name="fee_end_value[${productFeeCountFromDb}]" class="form-control productFeeUpdateForm onlyNumberInput" /></td>
                                             <td><input value="${
                                               row.fee
                                             }" type="number" name="fee_value[${productFeeCountFromDb}]" class="form-control productFeeUpdateForm onlyNumberInput" /></td>
                                             <td><input value="${
                                               row.min_fee ? row.min_fee : ""
                                             }" type="number" name="fee_min[${productFeeCountFromDb}]" class="form-control productFeeUpdateForm onlyNumberInput" /></td>
                                             <td colspan="2"><input value="${
                                               row.max_fee ? row.max_fee : ""
                                             }" type="number" name="fee_max[${productFeeCountFromDb}]" class="form-control productFeeUpdateForm onlyNumberInput" /></td>
                                         </tr>`;
...
function productFeeUpdateForm(className) {
  ...
  for (let i = 0; i < eleInputs.length; i++) {
    if (
      eleInputs.eq(i).hasClass("form-control-radio") &&
      eleInputs.eq(i)[0].checked
    ) {
