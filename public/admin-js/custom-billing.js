...
$(function () {
    ...
    $('#addNewRulesRow').on('click', function () {
        $('#addUpdateRulesModalTable tbody').append(addNewRulesRow(addUpdateRulesCountFromDb++));
    });

    //Bind update and change handler
    $('#addUpdateRulesModalTable').on('keyup change', '.addUpdateRulesInput', function () {
        formSubmitButtonHandler('addUpdateRulesInput');
    });
    $('#addUpdateRulesName').on('keyup change', '.addUpdateRulesInput', function () {
        formSubmitButtonHandler('addUpdateRulesInput');
    });
    ...
    $('#addNewSchemeAndRulesBtn').on('click', function () {
        ...
        $('#addUpdateRulesName').html(`<input type="text" class="form-control addUpdateRulesInput" name="scheme_name" placeholder="Enter scheme name here" />`);
        $('form[role="add-update-product-form"]').attr('action', $('meta[name="base-url"]').attr('content') + `/admin/custom-billing/scheme-rule/add-schemes`);
    ...
    $('#datatable_schemes_info').on('click', '.addUpdateSchemeAndRules', function () {
                    ...
                    switch (response.status) {
                        case 'SUCCESS':
                            ...
                            if (clickAction === 'update') {
                                $('#addUpdateRulesName').html(`<input type="hidden" name="scheme_id" value="${response.data.schemeId}" />
                                ...
                                trClass = 'fromBtn';
                                $('#addUpdateRulesName').html(`<input type="hidden" name="is_copy" value="copy_scheme" />
                            ...
                            if (response.data.schemeRules !== '' &&
                                ...
                                response.data.schemeRules.forEach((row, index) => {
                                    ...
                                    formInputRows += `
                                            ...
                                            <td>
                                                <select class="form-control addUpdateRulesInput form-control-select" data-value="${row.type}" name="type[${addUpdateRulesCountFromDb}]" required>
                                                    <option value="fixed" ${(row.type === 'fixed') ? 'selected' : ''}>Fixed</option>
                                                    <option value="percent" ${(row.type === 'percent') ? 'selected' : ''}>Percent</option>
                                                </select>
                                            </td>
                                            <td>
                                                <select class="form-control addUpdateRulesInput form-control-select" data-value="${row.is_active}" name="is_active[${addUpdateRulesCountFromDb}]" required>
                                                    <option value="1" ${(row.is_active === '1') ? 'selected' : ''}>Active</option>
                                                    <option value="0" ${(row.is_active === '0') ? 'selected' : ''}>Inactive</option>
                                                </select>
                                            </td>
                                            <td>
                                                ${(clickAction === 'update') ? `<input type="hidden" name="rule_id[${addUpdateRulesCountFromDb}]" value="${row.id}" />` : ''}
                                                <input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.start_value || ''}" name="start_value[${addUpdateRulesCountFromDb}]" />
                                            </td>
                                            <td><input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.end_value || ''}" name="end_value[${addUpdateRulesCountFromDb}]" /></td>
                                            <td><input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.fee || ''}" name="fee[${addUpdateRulesCountFromDb}]" /></td>
                                            <td><input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.min_fee || ''}" name="min_fee[${addUpdateRulesCountFromDb}]" /></td>
                                            ${(clickAction === 'update') ? `<td colspan="2"><input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.max_fee || ''}" name="max_fee[${addUpdateRulesCountFromDb}]" /></td>`
                                            : `<td><input type="number" min="0" class="form-control addUpdateRulesInput" value="${row.max_fee || ''}" name="max_fee[${addUpdateRulesCountFromDb}]" /></td>
                            ...
                            if (clickAction === 'update') {
                                $('form[role="add-update-product-form"]').attr('action', $('meta[name="base-url"]').attr('content') + `/admin/custom-billing/scheme-rule/edit-schemes`);
                            } else if (clickAction === 'copy') {
                                $('form[role="add-update-product-form"]').attr('action', $('meta[name="base-url"]').attr('content') + `/admin/custom-billing/scheme-rule/add-schemes`);
                            }
    ...
    $('#addUpdateRulesModalTable').on('change', '.updateRulesServiceId', function () {
        let srvId = $(this).val();
        let ele = $(this).parent().parent('tr').find('select.updateRulesProductId');
     ...
     */
    $('#datatable_scheme_user_relation').on('click', '.deleteUserSchemeRelation', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // 
                $.ajax({
                    url: $('meta[name="base-url"]').attr('content') + '/admin/custom-billing/delete/scheme-relation',
                    method: 'POST',
                    data: {
                    ...
                    success: (response) => {
                        if (response.status_code === '200') {
                            $('#datatable_scheme_user_relation').DataTable().ajax.reload(null, false);
            ...
            },
            {
                ...
                render: function (data, type, full) {
                    ...
                    if (full.is_assigned.length > 0) {
                        ...
                        return `<label class="switch" onChange="schemesActiveInactive('${full.id}')"><input type="checkbox" checked><span class="slider round"></span></label>`;
                    } else {
                        return `<label class="switch" onChange="schemesActiveInactive('${full.id}')"><input type="checkbox"><span class="slider round"></span></label>`;
                    }
                }
            },
            {
...
function schemesActiveInactive(id) {
    $.ajax({
        url: $('meta[name="base-url"]').attr('content') + `/admin/custom-billing/status/schemes`,
        method: 'POST',
        data: {
            schemeId: id,
            _token: $('meta[name="csrf-token"]').attr('content')
