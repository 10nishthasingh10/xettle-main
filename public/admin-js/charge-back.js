...
$(function () {
     ...
     */
    $('.addChargeInput').on('keyup', function () {

        let isButtonEnable = true;
        var eleInputs = $('.addChargeInput');
     ...
     * Edit Service Name 
     */
    $('#dt-charge-list').on('click', '.editCharge', function () {

        let rowId = $(this).attr('data-id');
        let txnName = $(this).attr('data-name');

        $('#update_row_id').val(rowId);
        $('#update_txn_id').val(txnId);
        updateServiceValue = txnName;

        $('#updateChargeModal').modal('show');
...
function addNewChargeCb(response) {
