    ...
    var options = [
      ...
      },
      {
        ...
        render: function (data, type, full, meta) {
          ...
          return `<span class="d-block text-right">${numberWithCommas(
      ...
      },
      {
        ...
        render: function (data, type, full, meta) {
          return `<span class="d-block text-right">${
            data !== null && data !== "" ? numberWithCommas(data.toFixed(2)) : 0
          }</span>`;
        },
      },
      {
...
$("#dt-last_txn").on("click", ".getServiceInfo", function () {
  ...
  if (!getServiceInfoAjax) {
    ...
    $.post(
      $('meta[name="base-url"]').attr("content") +
        "/admin/dashboard/active-user/txn-details",
      jsonData,
      (response) => {
  ...
  var options = {
    ...
    ajax: {
      ...
      data: function (d) {
        ...
        d.from = $("#latestUserForm")
          .find('[name="date-from_lst_signup"]')
          .val();
...
$(function () {
  ...
  $.get($("#dashboardBalances").val(), function (response) {
    ...
    $("#order-process-actual").html(
      `${response.orderProcessCount} | &#8377;${response.orderProcessActual}`
    );
  });
...
function handleFinancialOverviewCards(start, end, type) {
  ...
  getCardRecords("reseller/data/aeps/success", dateData);
  getCardRecords("reseller/data/aeps/pending", dateData);
  getCardRecords("reseller/data/aeps/failed", dateData);

  getCardRecords("reseller/data/validation/success", dateData);
  getCardRecords("reseller/data/validation/pending", dateData);
  getCardRecords("reseller/data/validation/failed", dateData);

  getCardRecords("reseller/data/recharge/processed", dateData);
  getCardRecords("reseller/data/recharge/processing", dateData);
  getCardRecords("reseller/data/recharge/failed", dateData);

  getCardRecords("reseller/data/dmt/processed", dateData);
  getCardRecords("reseller/data/dmt/failed", dateData);
  getCardRecords("reseller/data/dmt/processing", dateData);

  getCardRecords("reseller/data/matm/processed", dateData);
  getCardRecords("reseller/data/matm/failed", dateData);
  getCardRecords("reseller/data/matm/pending", dateData);

  getCardRecords("reseller/data/pancard/success", dateData);
  getCardRecords("reseller/data/pancard/failed", dateData);
  getCardRecords("reseller/data/pancard/pending", dateData);
}
...
function drawTransactionCharts(response) {
  ...
  $("#total-payout-chart")
    .html(`<i class="fas fa-circle text-bb"></i> ₹ ${changeNumberFormat(
    totSuccessAmt
  )}
    &nbsp;<i class="fas fa-circle text-rr"></i> ₹ ${changeNumberFormat(
      totFailedAmt
    )}
    &nbsp;<i class="fas fa-circle text-yy"></i> ₹ ${changeNumberFormat(
      totReversedAmt
    )}
    &nbsp;<i class="fas fa-circle text-dd"></i> ₹ ${changeNumberFormat(
      totProcessingAmt
    )}`);
  ...
  if (showGraph) {
    ...
    let DatePointX = payoutSuccess.sort(dynamicSort("z"));
    let DatePointArr = [];
    ...
    let payoutSuccessX = payoutSuccess.sort(dynamicSort("z"));
    let payoutSuccessArr = [];
    function payoutSuccessfunc(item, index) {
      payoutSuccessArr.push(parseInt(payoutSuccessX[index].y));
    }
    payoutSuccessX.forEach(payoutSuccessfunc);

    let payoutFailedX = payoutFailed.sort(dynamicSort("z"));
    let payoutFailedArr = [];
    function payoutFailedfunc(item, index) {
      payoutFailedArr.push(parseInt(payoutFailedX[index].y));
    }
    payoutFailedX.forEach(payoutFailedfunc);

    let payoutReversedX = payoutReversed.sort(dynamicSort("z"));
    let payoutReversedArr = [];
    function payoutReversedfunc(item, index) {
      payoutReversedArr.push(parseInt(payoutReversedX[index].y));
    }
    payoutReversedX.forEach(payoutReversedfunc);

    let payoutProcessingX = payoutProcessing.sort(dynamicSort("z"));
    let payoutProcessingArr = [];
    function payoutProcessingfunc(item, index) {
      payoutProcessingArr.push(parseInt(payoutProcessingX[index].y));
    }
    payoutProcessingX.forEach(payoutProcessingfunc);
      ...
      series: [
        {
          name: "Success",
          type: "area",
          data: payoutSuccessArr,
        },
        {
          name: "Failed",
          type: "line",
          data: payoutFailedArr,
        },
        {
          name: "Processing",
          type: "line",
          data: payoutProcessingArr,
        },
        {
          name: "Reversed",
          type: "line",
          data: payoutReversedArr,
        },
      ],
      fill: {
        colors: [
...
function drawModeCharts(response) {
  ...
  $("#payoutModeSuccess").html(
    `<canvas id="payoutModeSuccessCanvas" style="width:100%;max-width:600px"></canvas><div class="inside-donut-chart-label"><strong id="totPayoutModeSuccess" style="font-size: 18px;">0</strong></div>`
  );
  ...
  $("#totPayoutModeSuccess").text(
    "₹" + changeNumberFormat(totPayoutModeSuccess)
  );

  /**
   * Mode Failed
   */
  $("#payoutModeFailed").html(
    `<canvas id="payoutModeFailedCanvas" style="width:100%;max-width:600px"></canvas><div class="inside-donut-chart-label"><strong id="totPayoutModeFailed" style="font-size: 18px;">0</strong></div>`
  );
...
function drawAreaCharts(response) {
  ...
  $("#payoutAreaSuccess").html(
    `<canvas id="payoutAreaSuccessCanvas" style="width:100%;max-width:600px"></canvas><div class="inside-donut-chart-label"><strong id="totPayoutAreaSuccess" style="font-size: 18px;">0</strong></div>`
  );
   ...
   * Area Failed
   */
  $("#payoutAreaFailed").html(
    `<canvas id="payoutAreaFailedCanvas" style="width:100%;max-width:600px"></canvas><div class="inside-donut-chart-label"><strong id="totPayoutAreaFailed" style="font-size: 18px;">0</strong></div>`
  );
