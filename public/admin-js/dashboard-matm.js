...
$(function () {
    ...
    $('select#matm-user-id').on('change', function () {
        ...
        getMatmDashboardGraph(globalMatmStartDate, globalMatmEndDate, globalMatmFilterType);
    });
});
...
function getMatmDashboardGraph(start, end, type) {
    ...
    $.post($('#matmGraphs').val() + '/transaction', jsonData, function (response) {
        ...
        //Transaction counts
        $.post($('#matmGraphs').val() + '/txn-counts', jsonData, function (response) {
            if (response.code === '0x0200') {
...
function drawMatmTransactions(response) {
    ...
    $('#totalTxnMatmChart').html(`<i class="fas fa-square text-success bg-success"></i> ₹ ${changeNumberFormat(cwAepsSuccessTot)}
    ...
    if (showGraph) {
        ...
        $('#matmTxnChart').html(`<div class="position-relative mb-4">
            <canvas id="matmTxnChartCanvas" height="260"></canvas>
        </div>`);

        let matmTxnChartCanvas = $('#matmTxnChartCanvas');

        new Chart(matmTxnChartCanvas, {
            data: {
                labels: labels,
                datasets: [
                    {
...
function drawMatnCounts(response) {
    ...
    if (showGraph) {
        ...
        $('#matmCountChart').html(`<div class="position-relative mb-4">
            <canvas id="matmCountChartCanvas" height="260"></canvas>
        </div>`);

        let matmCountChartCanvas = $('#matmCountChartCanvas');

        new Chart(matmCountChartCanvas, {
            data: {
                labels: labels,
                datasets: [
                    {
