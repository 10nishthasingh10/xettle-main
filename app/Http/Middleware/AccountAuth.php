    ...
    public function handle($request, Closure $next)
    {
        ...
        if (isset($header)) {
            ...
            if (isset($header['php-auth-user'][0]) && isset($header['php-auth-pw'][0])) {
                ...
                $oAuthClient = OauthClient::select('user_id', 'service_id', 'client_key', 'client_secret', 'scope')->where([['client_key', $key], ['is_active', '1']])->first();
                if (isset($oAuthClient)) {
                    ...
                    if ($checkHash) {
                        if (User::where('id', '=', $oAuthClient->user_id)->where('is_active', '1')->count()) {
                            $GlobalConfig = GlobalConfig::select('attribute_1', 'attribute_2', 'attribute_3')->where(['slug' => 'payout'])->first();
                            ...
                            if ($isPayoutServiceEnable) {
                                $ipAddress = IpWhitelist::where([['ip', $ip], ['service_id', $oAuthClient->service_id], ['user_id', $oAuthClient->user_id], ['is_active', '1']])->first();
                                if (isset($ipAddress)) {

                                    $allowService = AllowedService::allowedService($oAuthClient->user_id, $oAuthClient->service_id, '11', Request::segment(3), $request);
