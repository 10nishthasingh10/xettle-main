 ...
 * MATMController
 */
class MATMSDKController extends Controller
{
        ...
        try {
            ...
            if ($validations['status'] == true) {
                ...
                if ($request->txnType == 'sdkenquiry') {
                    ...
                    $product = Product::where('slug', 'matm')->where('type', 'matm')->first();
                    if ($request->amount < 0) {
                        return Response::failed('The amount should be grater then 0.');
                    }

                    if ($product->min_order_value <= $request->amount && $product->max_order_value >= $request->amount) {
                        ...
                        return Response::failed($request->amount . ' Provided amount is not in range for Transaction');
                    }
                }
                ...
                $request->request->add(['routeType' => 'sdk']); 
                $body = [
                    "merchantcode" => $request->merchantCode,
            ...
            if ($validations['status'] == true) {
                ...
                $status  = 'pending';
                   ...
                   // $matm = DB::table('matm_transactions')->where(['user_id' => $request['auth_data']['user_id'], 'order_ref_id' => $response['response']['response']->clientrefid])->first();
                    ...
                    if ($response['response']['response']->statuscode == '000') {
                        MatmTransaction::updateRecord(
                            ['user_id' => $this->userId, 'client_ref_id' => $request->clientrefid],
                            [
                                'status' => 'processed',
        ...
        try {
            ...
            if ($validations['status'] == true) {
                ...
                if (isset($response['response']['response']->statuscode)) {
                    if ($response['response']['response']->statuscode == "000") {
                        MatmTransaction::updateRecord(
                            ['user_id' => $request['auth_data']['user_id'], 'order_ref_id' => @$txn->order_ref_id, 'status' => 'pending'],
                            [
                                'status' => 'processed',
                                'stanno' => @$response['response']['response']->stanno,
                                'rrnno' => @$response['response']['response']->rrnno,
    ...
    public static function getFeeAndTaxs($userId, $slug)
    {
        ...
        try {
            //code...
            $getProductId = CommonHelper::getProductId($slug, 'matm');
