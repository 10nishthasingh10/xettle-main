...
class Validation extends Model
{
     ...
     * @var array
     */
    protected $fillable = ['user_id', 'group_id', 'fee', 'tax', 'type', 'status', 'failed_messages', 'created_at', 'updated_at', 'product_id', 'request_id'];
    ...
    public static function create($userId, $orderArray, $type, $taxData = [])
    {
        ...
        try {
            // Transaction Create
            $orderData = [
                'root_type' => !empty($orderArray['rootType']) ? $orderArray['rootType'] : '1',
                'service_id' => VALIDATE_SERVICE_ID,
                'user_id' => $userId,
                'group_id' => @$orderArray['group_id'],
                'param_1' => @$orderArray['param1'],
                'param_2' => @$orderArray['param2'],
                'order_ref_id' => @$orderArray['task_id'],
                'type' => @$type,
                'status' => 'queued',
                'fee' => @$taxData['fee'],
                'tax' => @$taxData['tax'],
    ...
    public static function moveOrderToProcessingByOrderId($userId, $orderRefId, $fee, $tax, $trIdentifiers)
    {
        ...
        try {
            ...
            DB::select("CALL debitValidationBalanceOrder($userId, 0, $fee, $tax, '" . $orderRefId . "', '" . $txn . "','" . $trIdentifiers . "', @json)");
    ...
    public static function fundRefunded($userId, $orderRefId, $failedMessage, $trIdentifiers)
    {
        ...
        try {
            ...
            if (isset($OrderData) && !empty($OrderData)) {
                $txn = CommonHelper::getRandomString('txn', false);
                DB::select("CALL validationStatusUpdate('" . $OrderData->order_ref_id . "', $userId, $OrderData->id, '" . 'failed' . "', '" . $txn . "', '" . $failedMessage . "', '','','" . $trIdentifiers . "',  @json)");
                $results = DB::select('select @json as json');
                $response = json_decode($results[0]->json, true);
     ...
     * OCR Create
     */
    public static function ocrCreate($userId, $orderArray, $type, $taxData = [])
    {
        $response['status'] = false;
        $response['message'] = 'Order Not created';
        DB::beginTransaction();
        try {
            ...
            $orderData = [
                ...
                'param_1' => @$orderArray['data']['document1'],
                'param_2' => @$orderArray['data']['document2'],
    ...
    public static function ocrMoveOrderToProcessingByOrderId($userId, $orderRefId, $fee, $tax, $trIdentifiers)
    {
        $resp['status'] = false;
        $resp['message'] = 'Initiate';
    ...
    public static function ocrFundRefunded($userId, $orderRefId, $failedMessage, $trIdentifiers)
    {
        $resp['status'] = false;
        $resp['message'] = 'Initiate';
        try {
    ...
    public static function ocrUpdateRecord($cond = [], $data = [])
    {
        if (self::where($cond)->update($data)) {
            return true;
        }
