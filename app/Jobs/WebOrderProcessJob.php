...
class WebOrderProcessJob implements ShouldQueue
{
  ...
  /**
    ...
    {
        $fileName = 'public/weborderJobs'.$this->orderRefId.'.txt';
        ...
        try {
            ...
            ->first();
                if(isset($Order)) {
                    $userConfigGetRoute = CommonHelper::getPayoutRouteUsingUserId($Order->user_id, 'web');
                    if ($userConfigGetRoute['status']) {
                        $types = $userConfigGetRoute['slug'];
                        $integrationId = $userConfigGetRoute['integration_id'];
                    } else {
                        $route = CommonHelper::defaultPayoutRoute('payout_route');
                        $types = $route['slug'];
                        ...
                        case 'cashfree':
                            ...
                            $failedArray = array('403', '412', '422', '409', '404', '520', '429','400');
                                        ...
                                        if($lockeOrder['status'] && isset($Order)) {
                                            ...
                                            if (isset($requestTransfer['data']) && $requestTransfer['data'] != null)
                                            {
                                                    ...
                                                    ->subCode))
                                                {
                                                    ...
                                                    if (in_array($requestTransfer['data']->subCode, $failedArray)) {
                                                        if ($requestTransfer['data']->subCode == '400') {
                                            ...
                                            BulkPayoutDetail::payStatusUpdate($Order->batch_id, 'failed', $Order->order_ref_id, $lockeOrder['message'], '');
                                        }
                                    } else{
                            ...
                            if ($lockeOrder['status'] && isset($Order)) {
                                ...
                                if (isset($requestTransfer['data']) && $requestTransfer['data'] != null) {
                                    ...
                                    if (isset($requestTransfer['data']->statuscode)) {
                                        $errorDesc = @$requestTransfer['data']->status;
                                        ...
                                        } else if (isset($requestTransfer['data']->statuscode) && in_array($requestTransfer['data']->statuscode, $failedArray)) {
                                            $errorDesc = isset($requestTransfer['data']->status) ? $requestTransfer['data']->status : $errorDesc;
                                            if (in_array($requestTransfer['data']->statuscode, ['IAB'])) {
                                                $errorDesc = 'Something went wrong, please try after some time';
                                            }

                                            $statusCode = $requestTransfer['data']->statuscode;
                                            $txn = CommonHelper::getRandomString('txn', false);
                                            $utr = $bank_reference = @$requestTransfer['data']->data->txnReferenceId;
                                            $getServicePkId = DB::table('user_services')->select('id')->where('user_id', $Order->user_id )->where('service_id', PAYOUT_SERVICE_ID)->first();
                                            DB::select("CALL OrderStatusUpdate('".$Order->order_ref_id."', $Order->user_id, $getServicePkId->id, 'failed', '".$txn."', '".$errorDesc."', '".$statusCode."','".$utr."', @json)");
                                            $results = DB::select('select @json as json');
                                            $response = json_decode($results[0]->json, true);
                                ...
                                $messages = 'Order already processed';
                                if ($lockeOrder['message'] == 'debit_balance_failed') {
                                    dispatch(new \App\Jobs\WebOrderProcessJob($Order->order_ref_id, $Order->user_id))->delay(rand(1, 10))->onQueue('bulk_payout_queue');
                                }
                                        ...
                                        if (isset($requestTransfer['data']) && $requestTransfer['data'] != null)
                                        {
                                                ...
                                                ->success == false)
                                            {
                                                ...
                                                if($response['status'] == '1') {
                                                    ...
                                                    ->failure)) {
                                                $errorDesc = isset($requestTransfer['data']->message) ? $requestTransfer['data']->message : "Order failed";
                                                $statusCode = '';
                                                $orderStatus = 'failed';
                                                $txn = CommonHelper::getRandomString('txn', false);
                            ...
                            if($lockeOrder['status'] && isset($Order)) {
                                ...
                                if (isset($requestTransfer['data']) && $requestTransfer['data'] != null)
                                {
                                                            ...
                                                            ->status))
                                    {
                                        ...
                                        if (isset($requestTransfer['data']
                                        ) && in_array($requestTransfer['data']
                                        ->status, $failedArray)) {
                                            $errorDesc = $errorMsg = isset($requestTransfer['data']->error->reason) ? $requestTransfer['data']->error->reason : $requestTransfer['data']->error->description;
                                            $statusCode = '003';
    ...
    {
        //$time = [5];
        return [(new WithoutOverlapping($this->userId))->releaseAfter(rand(1, 20))];
    }
     ...
     * @return void
     */
    public static function updateStatusByBatchId($batchId, $data)
    {
