...
class MatmTransactionAmountJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
                ...
                $transaction = MatmTransaction::groupBy('id')
                    ->where(['user_id' => $this->userId, 'is_trn_credited' => '0', 'status' => 'processed'])
                ...
                if (isset($transaction) && count($transaction)) {
                    ...
                    $txnRefId = CommonHelper::getRandomString('MAST', false);
                    $idOfAepsTransaction = implode(',', $idArray);

                    DB::select("CALL matmCreditAmountTransaction($this->userId,  'srv_1680249103', '".$txn."', '".$txnRefId."', 'matm_inward_credit', $fee, $tax, $totalAmount , $finalAmount, '$idOfAepsTransaction', @json)");
                    ...
                    if($response['status'] == '1') {
                        ...
                        if (!empty($user->email)) {
                            try {
                                ...
                                dispatch(new SendTransactionEmailJob((object) $mailParms, 'sendMATMCreditTxn'));
                            } catch (\Exception $e) {
                                //mail not send
            ...
            $fileName = 'public/MatmTransaction'.$this->userId.'.txt';
