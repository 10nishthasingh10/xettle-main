...
class CallbackRazorpayOrderUpdate implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            //Order Success
            if ('processed' == $this->event) {
                $callBackData = DB::table('rp_payout_callbacks')
                ...
                if ($callBackData) {
                    ...
                    $statusCode = 200;
                    $bank_reference = $responseJson['payload']['payout']['entity']['utr'];
                    $Order = DB::table('orders')
                        ->where('order_ref_id', $responseJson['payload']['payout']['entity']['reference_id'])
                        ->select('user_id','order_ref_id', 'batch_id')
                        ->first();
                    DB::select("CALL OrderStatusProcessedUpdate('".$responseJson['payload']['payout']['entity']['reference_id']."', $Order->user_id, 'processed', '".$message."', '".$statusCode."', '".$bank_reference."', @json)");
            ...
            // Order Failed
            if ('rejected' == $this->event || 'cancelled' == $this->event || 'failed' == $this->event) {
                $callBackData = DB::table('rp_payout_callbacks')
                    ->where('event', $this->event)
                    ->where('txnid', $this->orderRefId)
                    ->where('is_refunded', '0')
                ...
                if ($callBackData) {
                    ...
                    $Order = DB::table('orders')
                        ...
                        ->select('user_id', 'order_ref_id', 'batch_id')
                        ->first();
                    $errorDesc = isset($responseJson['payload']['payout']['entity']['failure_reason']) ? $responseJson['payload']['payout']['entity']['failure_reason'] : "Order failed";
                    $statusCode = '';
                    $referenceId = isset($responseJson['payload']['payout']['entity']['reference_id']) ? $responseJson['payload']['payout']['entity']['reference_id'] : " ";
                    $utr = isset($responseJson['payload']['payout']['entity']['utr']) ? $responseJson['payload']['payout']['entity']['utr'] : $referenceId;
              ...
              if ('reversed' == $this->event) {
                ...
                if ($callBackData) {
                    ...
                    if (isset($userConfig) && !empty($userConfig)) {
                        $errorDesc = isset($responseJson['payload']['payout']['entity']['failure_reason']) ? $responseJson['payload']['payout']['entity']['failure_reason'] : "Order reversed";
                        $statusCode = '';
