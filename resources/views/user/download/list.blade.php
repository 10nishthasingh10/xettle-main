                            ...
                            <div class="col-md-2">
                                ...
                                <label>Get Report <span class="requiredstar"></span></label>
                                        <select name = "reports" class="form-control js-example-basic-multiple">
                                            ...
                                            <!-- <option value = "Summary" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'Summary')) selected @endif> Summary </option> -->
                                            <option value = "Smart_Payout" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'Smart_Payout')) selected @endif> Smart Payout </option>
                                                <!-- <option value = "Contacts" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'Contacts')) selected @endif> Contacts </option> -->
                                            <option value = "UPI_Stack" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'UPI_Stack')) selected @endif> UPI Stack </option>
                                                <option value = "Smart_Collect" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'Smart_Collect')) selected @endif> Smart Collect </option>
                                            <option value = "Patner_VAN" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'Patner_VAN')) selected @endif> Patner VAN </option>
                                                <option value = "AEPS" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'AEPS')) selected @endif> AEPS </option>
                                            <option value = "AutoSettlement" @if(isset($_GET['reports'])
                                                && in_array($_GET['reports'], 'AutoSettlement')) selected @endif> AutoSettlement </option>
...
<script>
    $(document).ready(function() {
        ...
        let currDate = `{{date('Y-m-d')}}`;
        $('#toDate').attr('max', currDate);
        $('#fromDate').attr('max', currDate);


        var url = "{{custom_secure_url('user/fetch')}}/exceldownload/{{Auth::user()->id}}";
        var onDraw = function() {};
        var options = [ {
                "data": "file_name"
            },
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    if (file == false) {
                        return '<span class="edit btn btn-primary btn-sm"><a href="/user/allreports/excelDownloadLink/'+full.id+'"  tooltip="Download"  style="color:white;text-decoration:none" ><i class="os-icon os-icon-download"></i></a></span><span class="edit btn btn-danger btn-sm"><a href="javascript:void(0);" onclick="deleteFile(\'' + full.id + '\')" tooltip="Delete File"  style="color:white;text-decoration:none" data-toggle="modal"><i class="os-icon os-icon-x-circle"></i></a></span>';
                    } else {
                        return '';
                    }
                }
...
<script type="text/javascript">
    ...
    frm.submit(function (e) {
        ...
        var formData = new FormData(this);
            $.ajax(
                {
                    url: "{{url('/user/allreports/ajaxGenerateExcelFile')}}",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
    ...
    function getExcelDataCount(count, interval) {
        $.ajax({
                type: "get",
                url: "{{url('user/allreports/getCountRecord')}}",
    ...
    function deleteFile(id)
    {
        ...
        {
            $.ajax({
                url:"{{custom_secure_url('user/removeExportFile/')}}"+"/"+id,
                type:"POST",
