                ...
                <form id="searchForm">
                    ...
                    <fieldset class="form-group">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="form-control js-example-basic-multiple" multiple="" id="services" name="services[]">
                                        ...
                                        <option value="{{$serviceListObjects->id}}">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                <label for="">Filter by Pipe:</label>
                                    <select  class="form-control js-example-basic-multiple" multiple="" name="allpipe" id="allpipe[]">
                                        @foreach($Pipe as $pipes)
                                        <option value="{{$pipes->id}}">
                                            {{$pipes->pipeName}} 
                                        </option>
                                        @endforeach
                                    </select>
...
<div aria-hidden="true" aria-labelledby="exampleModalLabel" class="modal fade" id="userStatusChange" data-DataTablesd="datatable" role="dialog" tabindex="-1">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="orderForm" role="cancel-request-form" action="{{url('admin/users/statusChange')}}" data-DataTables="datatable" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input class="form-control" type="hidden" name="logged_id" id="logged_id" value="{{encrypt(Auth::user()->id)}}" />
                    </div>
                </div>
...
<script src="{{asset('admin-js/user-list.js')}}"></script>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var url = "{{custom_secure_url('admin/fetch')}}/users/0";
        var onDraw = function() {};
        var options = [{
            ...
            },
            {
                ...
                "render": function(data, type, full, meta) {
                    if (full.integration) {
                        let integrationInfo = '';
                        integrationInfo += `Integration Type: ${full.integration.type}<br>`;
                        integrationInfo += `Integration Key: ${full.integration.key}<br>`;
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    if ("{{Auth::user()->hasRole('super-admin')}}") {
                        ...
                        if (full.is_active == 0) {
                            isActive += `<option value="{{encrypt(0)}}" selected> Initiate </option>`;
                        } else {
                            isActive += `<option value="{{encrypt(0)}}"> Initiate </option>`;
                        }
                        if (full.is_active == '1') {
                            isActive += `<option value="{{encrypt(1)}}" selected> Active </option>`;
                        } else {
                            isActive += `<option value="{{encrypt(1)}}"> Active </option>`;
                        }

                        if (full.is_active == 2) {
                            isActive += `<option value="{{encrypt(2)}}" selected> Inactive </option>`;
                        } else {
                            isActive += `<option value="{{encrypt(2)}}"> Inactive </option>`;
                        }
                        if (full.is_active == '3') {
                            isActive += `<option value="{{encrypt(3)}}" selected> Suspended </option>`;
                        } else {
                            isActive += `<option value="{{encrypt(3)}}" > Suspended </option>`;
                        }
                        if (full.is_active == '4') {
                            isActive += `<option value="{{encrypt(4)}}" selected>  Blocked </option>`;
                        } else {
                            isActive += `<option value="{{encrypt(4)}}">  Blocked </option>`;
                        }
                        isActive += `</select> </br> `;
                        var $viewOrder = 'user/' + full.id;
                        var $actionBtn = isActive + ' <a href="' + $viewOrder + '" target="_blank" title="View User" class="edit btn btn-primary btn-sm" ><i class="os-icon os-icon-eye"></i></a><a href="userprofile/'+full.id+'" target="_blank" class="edit btn btn-primary btn-sm"><i class="os-icon os-icon-edit"></i></a><a target="_blank" href="user/userdetails/' + full.id + '" class="edit btn btn-primary btn-sm"><i class="os-icon os-icon-external-link"></i></a>';
                        return `<span class='xttl-inline-flex'>${$actionBtn + usrConfig}</span>`;
                    } else if (full.business_name != null && full.business_name != undefined && "{{Auth::user()->hasRole('support')}}" && full.business_name.is_kyc_updated == '1') {
                        var $viewUserDetails = "{{url('admin/profileOpenByAdmin')}}/" + full.id;
        ...
        var options = {
            ...
            ajax: {
                ...
                data: function(d) {
                    ...
                    d.allpipe = $('#searchForm').find('[id="allpipe"]').val();
                },
                beforeSend: function() {},
                complete: function() {
