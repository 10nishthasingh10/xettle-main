...
<div class="content-w">
    ...
    @include(USER.'.payout.modals.settlementlogs')
    @section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            var url = "{{custom_secure_url('admin/fetch')}}/settlementorders/0";
            var onDraw = function() {};
            var options = [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        if (data == 'failed' && "{{Auth::user()->hasRole('super-admin')}}") {
                        ...
                        if (data == 'processing' && "{{Auth::user()->hasRole('super-admin')}}") {
        ...
        function settlementHoldOrder(id, txnRefId) {
            $.ajax({
                url: "{{url('admin/settlementStatus')}}/" + id + '/' + txnRefId,
                type: 'GET',
    ...
    <script type="text/javascript">
        ...
        $('#processingAutoSettlementOrderUpdate').on('click', function() {
            $.ajax({
                url: "{{url('admin/processingAutoSettlementOrderUpdate')}}",
                type: 'GET',
                success: function(res) {
                    // console.log(res);
                    alert(res);
